...
def main(tile_file):
    ...
    for tile in tile_list:
        ...
        # If the zarr file exists,print
            ...
            test_files= os.listdir(f"/app/temp_data/fua")   
            #zarr_path=Path("riacs/fua/fua_63000308_h11v07.zarr")
            zarr_path=Path(f"/app/temp_data/fua/fua_{poly_id}_{tile}.zarr")
            ...
            #poly_zarr = zarr.open(str(zarr_path), mode='r')
            poly_zarr = zarr.open(f"/app/temp_data/fua/fua_{poly_id}_{tile}.zarr", mode='r')  
                
            ts_stack, dates=zarr_to_numpy(zarr_path,poly_zarr)# remove ra_ntls
                
            with open(str(Path("/app/temp_data/fua",f"fua_{poly_id}_{tile}_obs.npy")), 'wb') as f:
                np.save(f, ts_stack)
                
            with open(str(Path("/app/temp_data/fua",f"fua_{poly_id}_{tile}_date.npy")), 'wb') as f:
                np.save(f, np.asarray(poly_zarr["Dates"][0:]))
            
            np.savetxt(str(Path("/app/temp_data/fua",f"fua_{poly_id}_{tile}_obs2.csv")),ts_stack,fmt='%f', delimiter=',', newline='\n',header='ntl_avg, ntl_wt_avg, gf_avg, gf_wt_avg,gf_flag, ntl_flag')
                
            print(f"done converting {poly_id} zarr to numpy")
                
            # Creating zarr files for WSF and VNP46A2        
                
            test_files1= os.listdir(f"/app/temp_data/fua") 
            
            print('test_files1:', test_files1)
