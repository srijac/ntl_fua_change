...
def main():
    ...
    w_dir_zarr=f"ceph:multimodal"
    w_dir_zarr_e=f"ceph:multimodal-east"
    w_dir_zarr_sup=f"ceph:fua-boundary"
    w_dir_zarr_sup_e=f"ceph:fua-boundary-east"
    w_dir_comb=f"ceph:anthropause"
    w_dir_comb_e=f"ceph:anthropause-east"
    w_dir_fua=f"ceph:fua-subset-numpy"
    w_dir_fua_e=f"ceph:fua-subset-numpy-east"
    w_dir_anom=f"ceph:anomaly-write"
    w_dir_anom_e=f"ceph:anomaly-write-east"
    w_dir_a2=f"ceph:vnp46a1-thermal-anomaly"
    w_dir_a2_e=f"ceph:vnp46a1-thermal-anomaly-east"
    w_dir_a2=f"ceph:vnp46a1-thermal-anomaly"
    w_dir_a2_e=f"ceph:vnp46a1-thermal-anomaly-east"
    w_dir_wf=f"ceph:vnp09ga"
    w_dir_wf_e=f"ceph:vnp09ga-east"
    #w_dir_list2=f"ceph:zarrs/daily_change"
    for new_dir in [w_dir_zarr, w_dir_zarr_e,w_dir_zarr_sup, w_dir_zarr_sup_e,w_dir_comb, w_dir_comb_e, w_dir_fua, w_dir_fua_e,w_dir_anom, w_dir_anom_e,w_dir_a2, w_dir_a2_e,w_dir_wf, w_dir_wf_e]:
        create_s3_dir(cfg, new_dir)
    ...
    rclone.with_config(cfg).run_cmd(command="copy", 
                                                extra_args=[str(Path(f"{w_dir_wf}/",f"dummy_write_bucket.txt")),
                                                            f"/app/temp_data/"])
                                                            
    with open((f"/app/temp_data/dummy_write_bucket.txt"),'r') as t:
        print('reading wf', t.read())
        t.close()
        
    os.remove(str(Path(f"/app/temp_data", f"dummy_write_bucket.txt")))
    
    rclone.with_config(cfg).run_cmd(command="copy", 
                                                extra_args=[str(Path(f"{w_dir_wf_e}/",f"dummy_write_bucket.txt")),
                                                            f"/app/temp_data/"])
                                                            
    with open((f"/app/temp_data/dummy_write_bucket.txt"),'r') as t:
        print('reading wfe', t.read())
        t.close()
